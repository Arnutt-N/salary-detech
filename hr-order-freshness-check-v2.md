...
| ให้ได้รับเงินเดือน (ผู้บริหาร/ผู้ทรงคุณวุฒิ) | การเคลื่อนไหวตำแหน่ง |
| ให้ได้รับเงินเดือนตามคุณวุฒิ | การเคลื่อนไหวเงินเดือน |

---

## 2.1 ฟิลด์ snapshot และคำศัพท์มาตรฐาน (Import / DB)

> ใช้คำศัพท์ต่อไปนี้เท่านั้น — **ห้าม**ใช้คำย่อผิด: ~~เงินเพิ่มครองชีพ~~, ~~ค่าตอบแทนนอกเหนือ~~

### องค์ประกอบเงินเดือนในคำสั่ง

| ชื่อไทย (มาตรฐาน) | คอลัมน์ DB (`orders`) | หมายเหตุ |
|---|---|---|
| เงินเดือน | `salary` | ฐานเงินเดือนในคำสั่ง |
| **เงินเพิ่มการครองชีพชั่วคราว** | `cost_of_living_allowance` | ไม่ใช่ "เงินเพิ่มครองชีพ" |
| ค่าตอบแทนพิเศษ | `special_compensation` | ต่างจากตาราง `compensation_rounds` (ติดตามรอบ 6 เดือน) |
| เงินประจำตำแหน่ง | `position_allowance` | ใช้ในคำสั่งเคลื่อนไหวตำแหน่ง |
| **ค่าตอบแทนนอกเหนือจากเงินเดือน** | `compensation_beyond_salary` | ไม่ใช่ "ค่าตอบแทนนอกเหนือ" |
| เงินเดือน ณ วันที่ | `salary_as_of_date` | anchor รอบ 1 เม.ย. / 1 ต.ค. ตาม scenario |
| วันที่มีผล | `effective_date` | |
| หมายเหตุ | `note` (อนาคต) | ยังไม่มีใน schema ปัจจุบัน |

### แผ่น import ใน Excel (`docs/templates/template-salary-detect.xlsx`)

**คำสั่งเคลื่อนไหวตำแหน่ง** (`movement`) — มีคู่ **เดิม / ที่แต่งตั้ง**:

| หัวคอลัมน์ Excel (ตัวอย่าง) | ฝั่ง | ฟิลด์หลัก |
|---|---|---|
| เงินเดือนที่เดิม, เงินเพิ่มการครองชีพชั่วคราวที่เดิม, … | เดิม | `prior_*` (วางแผนใน schema รุ่นถัดไป) |
| เงินเดือนที่แต่งตั้ง, เงินเพิ่มการครองชีพชั่วคราวที่แต่งตั้ง, … | ใหม่ | คอลัมน์ `orders` ด้านบน |
| เงินประจำตำแหน่ง, ค่าตอบแทนนอกเหนือจากเงินเดือน | ใหม่ | `position_allowance`, `compensation_beyond_salary` |

**คำสั่งเลื่อนเงินเดือน / ลาออก / ตามคุณวุฒิ** (`salary`, `resign`, ฯลฯ) — snapshot จุดเดียว:

ชื่อ-นามสกุล, ตำแหน่ง, สังกัด (กระทรวง→กรม→สำนัก/กอง→ต่ำกว่า 1 ระดับ), ประเภท/ระดับตำแหน่ง, เลขที่ตำแหน่ง, เงินเดือน, **เงินเพิ่มการครองชีพชั่วคราว**, ค่าตอบแทนพิเศษ, เงินเดือน ณ วันที่, วันที่มีผล, หมายเหตุ

### การ map `order_type` (แนะนำ)

| ประเภทธุรกิจ | `order_type` |
|---|---|
| เลื่อนระดับ | `promotion` |
| แต่งตั้ง | `appointment` |
| ย้าย / รับโอน / โอน / ให้โอน | `transfer` / `transfer_in` / `transfer_out` / `assign_transfer` |
| ลาออก | `resign` |
| ให้ได้รับเงินเดือน (movement) | `salary_entitlement` |
| เลื่อนเงินเดือนปกติ | `salary_increase` |
| เลื่อนเงินเดือนกรณีพิเศษ | `special_salary` |
| ปรับ 1 พ.ค. 67/68 | `salary_cap_adjustment` |
...
|---|---|---|---|
| เงินเดือน | `salary`, `salary_as_of_date`, `cost_of_living_allowance`, `special_compensation`, `position_allowance`, `compensation_beyond_salary` | `status_salary` | เลื่อนเงินเดือน, เลื่อนตำแหน่ง, ปรับวุฒิ, ปรับอัตราทั่วประเทศ, ค่าตอบแทนพิเศษ → เลื่อนเงินเดือน (องค์ประกอบย่อย — รองรับใน schema; engine ตรวจ `salary` + `salary_as_of_date` ก่อน) |
...
CREATE TABLE orders (
    ...
    cost_of_living_allowance     DECIMAL(10,2),   -- เงินเพิ่มการครองชีพชั่วคราว
    special_compensation         DECIMAL(10,2),   -- ค่าตอบแทนพิเศษ
    position_allowance           DECIMAL(10,2),   -- เงินประจำตำแหน่ง
    compensation_beyond_salary   DECIMAL(10,2),   -- ค่าตอบแทนนอกเหนือจากเงินเดือน
    salary_as_of_date            DATE,            -- เงินเดือน ณ วันที่
